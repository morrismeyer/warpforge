...
TARGET_MAC="10:ff:e0:c4:be:75"       # <-- nvidia MAC (enp8s0)
...
BUILD_CMD="${BUILD_CMD_OVERRIDE:-./gradlew clean assemble nvidiaTest}"
...
if [[ -x "$HOME/ci/wake-nvidia.sh" ]]; then
  ...
  if command -v wakeonlan >/dev/null 2>&1; then
    ...
    wakeonlan -i 192.168.1.160 "$TARGET_MAC" | tee -a "$LOG_FILE"
  else
