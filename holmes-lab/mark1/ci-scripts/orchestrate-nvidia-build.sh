...
# Orchestrated build runner for the NVIDIA box.
#
# Intended to run ON the NUC. It:
# - Wakes the NVIDIA box
# - Waits for SSH
# - Updates the WarpForge repo on the NVIDIA box
# - Runs build + tests on the NVIDIA box
...
TARGET_HOST="${TARGET_HOST_OVERRIDE:-nvidia}"
TARGET_MAC="${TARGET_MAC_OVERRIDE:-10:7C:61:3D:E7:8F}"
...
# Path to the repo on the NVIDIA box
...
WAKE_SCRIPT="${WAKE_SCRIPT_OVERRIDE:-$SCRIPT_DIR/wake-nvidia.sh}"
...
ssh "$TARGET_HOST" bash -lc "
  ...
  bash -lc \"./gradlew :ptest:validateJfrNvidia --no-configuration-cache\"

  # Check for success marker (written if GPU events were captured)
  if [ -f ptest/build/jfr-nvidia.success ]; then
    ...
    echo \"[remote \$(date)] JFR validation skipped (CUDA not available or no events captured)\"
  fi
